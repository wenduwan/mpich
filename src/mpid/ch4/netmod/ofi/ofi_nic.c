...
int MPIDI_OFI_init_multi_nic(struct fi_info *prov)
{
    ...
    MPIR_ERR_CHKANDJUMP(!MPIDI_OFI_global.prov_use || !MPIDI_OFI_global.nic_info, mpi_errno, MPI_ERR_OTHER, "**nomem");
    ...
    MPIDI_OFI_global.ctx = MPL_calloc(
        MPIDI_OFI_global.num_nics * MPIDI_CH4_MAX_VCIS,
        sizeof(MPIDI_OFI_context_t),
        MPL_MEM_OTHER);
...
static int setup_multi_nic(size_t nic_count)
{
    ...
    MPIR_CHKLMEM_MALLOC(parents, MPIR_hwtopo_gid_t *,
                        ...
                        mpi_errno, "temporary nic hwtopo object",
                        MPL_MEM_ADDRESS);
